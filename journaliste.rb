...
def question1 ( handle = tableau )

  puts "la réponse est : #{handle.length}" 
end

def question2 ( handle = tableau )
  puts "la réponse est : #{handle.min{|a,b| a.size <=> b.size}}"
end 

def question3 ( handle = tableau )
  puts "la réponse est : #{handle.count{|a| a.length == 6}}"
end

def question4 ( handle = tableau )
  puts "la réponse est : #{handle.count {|e| e =~ /^@[A-Z]/}}"
end

def question5 ( handle = tableau )
  puts "la réponse est : #{handle.sort_by { |a| a.downcase }}"
end

def question6 ( handle = tableau )
  puts "la reponse est : #{handle.sort_by {|x| x.length}}"
end


def question7 ( handle = tableau )
  puts "la réponse est : #{handle.index('@epenser')}"
end

def question8 ( handle = tableau ) 
puts " jsuis mort j'ai rien trouvé"
end
...
puts "quelle question choisis tu ? ( exemple > tape 1 pour la question 1)"
print "question numeros ? >"
number = 0
...
while number > 1 || number < 8 
 ...
 if number == 1
   ...
   question1 
     ...
     puts "il ny'a pas de question #{number} question numeros ? >"
     number = gets.chomp.to_i
